...
namespace ODataService.Client
{
    class Program
    {
        ...
        private static void Delete_Product_link_Family()
        {
            ...
            var product = container.Products.AsEnumerable().First();
            container.LoadPropertyAsync(product, "Family");
        ...
        private static void Put_ProductFamily()
        {
            ...
            if (family != null)
            {
                ...
                container.SaveChangesAsync(SaveChangesOptions.ReplaceOnUpdate);
            }
            else
            {
                Console.WriteLine("\tProductFamily with Id '{0}' not found.", key);
        ...
        private static void Invoke_Action()
        {
            ...
            var results = container.ExecuteAsync<int>(new Uri(uri), "POST", true, new BodyOperationParameter("Name", "New Product")).Result;
