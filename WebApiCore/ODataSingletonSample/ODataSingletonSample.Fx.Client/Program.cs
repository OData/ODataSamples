...
using ODataSingletonSample.Fx.Client;

namespace SingletonClientSample.Client
{
    public class Program
    {
        private static readonly string _baseAddress = "http://localhost:2629/odata";  //Set to http://localhost.fiddler:2629/odata if need to capture request in fiddler
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Once started, press any key to continue...\n");
            Console.ReadKey();
            ...
            // Query Singleton
            var company = clientContext.Umbrella.GetValue();
            ...
            company = clientContext.Umbrella.GetValue();
            ...
            // Query Option - $select
            Console.WriteLine("Execute: GET http://localhost:2629/odata/Umbrella?$select=Name,Revenue");
            clientContext.Umbrella.Select(c => new Company { Name = c.Name, Revenue = c.Revenue }).GetValue();

            // Query Option - $expand
            Console.WriteLine("Execute: GET http://localhost:2629/odata/Umbrella?$expand=Employees");
            clientContext.Umbrella.Expand(c => c.Employees).GetValue();

            // Delete navigation link
            Console.WriteLine("Delete navigation link: DELETE http://localhost:2629/odata/Umbrella/Employees/$ref?$id=http://localhost:50268/odata/Employees(1111) ");
