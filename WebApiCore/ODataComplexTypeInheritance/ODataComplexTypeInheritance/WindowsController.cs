...

{
    public class WindowsController : ODataController
    {
        ...
        [ODataRoute("Windows({key})/CurrentShape/ODataComplexTypeInheritanceSample.Models.Circle/Radius")]
        ...
        [ODataRoute("Windows({key})/GetTheLastOptionalShape()")]
        public IActionResult GetTheLastOptionalShape(int key)
        {
            Window window = _windows.FirstOrDefault(e => e.Id == key);
            if (window == null)
            {
        ...
        public IActionResult AddOptionalShape(int key, ODataActionParameters parameters)
        {
