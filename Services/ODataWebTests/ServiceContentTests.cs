...
namespace ODataWebTests
{
    ...
    {
#if DEBUG
        String nwServiceRoot = "http://localhost:51514/V2/Northwind/Northwind.svc/";
        String odServiceRoot = "http://localhost:51514/V2/(S(sessionid))/OData/OData.svc/";
#else
        String nwServiceRoot = "http://services.odata.org/V2/Northwind/Northwind.svc/";
        String odServiceRoot = "http://services.odata.org/V2/(S(sessionid))/OData/OData.svc/";
#endif
        ...
        private Stream MakeRequest(String requestUri, string accept, string method, string body)
        {
            ...
            if (body != null)
            {
                request.ContentType = accept == "application/json" ? "application/json" : "*/*";
        ...
        {
            return GetResponseAsText(requestUri, accept, "GET", null);
        }
        ...
        public void FormatEndPointTest()
        {
            ...
            {
                string jsonBaseline = GetResponseAsText(serviceBaseUri + "Products", "application/json");
                string jsonResult = GetResponseAsText(serviceBaseUri + "Products?$format=json");
        ...
        {
            string payload = "{ \"Rating\": { \"Name\": \"Bart\" } }";
            InvalidOperationException ex = null;
            try
            {
                string response = GetResponseAsText(odServiceRoot + "Products(0)", "application/json", "PUT", payload);
            }
            ...
            Assert.IsNotNull(ex);
            Assert.IsTrue(ex.Message.Contains("An unexpected 'StartObject' node was found when reading from the JSON reader. A 'PrimitiveValue' node was expected."));
        }
