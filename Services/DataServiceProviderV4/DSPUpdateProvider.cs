        ...
        protected virtual void ApplyResourceDefaultValue(DSPResource resource)
        {
            if (resource != null)
            {
                foreach (var prop in resource.ResourceType.Properties)
                {
                    ...
                    if (resource.ResourceType.Name == "Advertisement")
                    {
                        if (prop.Name == "ID")
                        {
                            resource.SetValue(prop.Name, Guid.NewGuid());
                        }
                        if (prop.Name == "AirDate")
                        {
                            resource.SetValue(prop.Name, new DateTime(2000, 1, 1));
                        }
                    }
                }
            }
